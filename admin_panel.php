...
<header>
                                ...
                                echo '</tr>';
                                 foreach ($data as $row) {
                                    ...
                                    if ($row['status'] == 'confirmed') {
                                        echo '<td><button class="button_av" onclick="completeReq(' . $row['request_id'] . ')">Complete</button> <button class="button_av" onclick="cancelReq(' . $row['request_id'] . ')">Cancel</button></td>';
                                    } elseif ($row['status'] == 'completed') {
                                        echo '<td>&#10004;</td>';
                                    } elseif ($row['status'] == 'cancelled') {
                                        echo '<td>&times;</td>';
                                    }elseif ($row['status'] == 'pending') {
                                        echo '<td><button class="button_av" onclick="confirmReq(' . $row['request_id'] . ')">Confirm</button> <button class="button_av" onclick="cancelReq(' . $row['request_id'] . ')">Cancel</button></td>';
                                    }
                                    echo '</tr>';
                                }
                                echo '</table>';
    ...
    function cancelReq(reqId) {
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'cancel_req.php', true); 
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onload = function () {
            if (xhr.status >= 200 && xhr.status < 400) {
        ...
        xhr.send('request_id=' + reqId);
    }

    function completeReq(reqId) {
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'complete_req.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onload = function () {
            if (xhr.status >= 200 && xhr.status < 400) {
                alert('Request completed successfully!');
            } else {
                alert('Error completing Request. Please try again later.');
            }
        };
        xhr.onerror = function () {
            alert('Error completing request. Please try again later.');
    ...
    function confirmReq(reqId) {
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'confirm_req.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onload = function () {
            if (xhr.status >= 200 && xhr.status < 400) {
                alert('Request confirmed successfully!');
            } else {
                alert('Error confirming request. Please try again later.');
            }
        };
        xhr.onerror = function () {
            alert('Error confirming request. Please try again later.');
